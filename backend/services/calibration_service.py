...
def validate_polygon(
    polygon: List[List[int]], width: int | None = None, height: int | None = None
) -> None:
    if len(polygon) < 3:
    ...
    if (
        ...
        and any(x < 0 or y < 0 or x > width or y > height for x, y in polygon)
    ):
        raise ValueError("Polygon vertex lies outside the camera frame")


def save_version(
    db, camera_id: int, user_id: int | None, frame: np.ndarray | None, note: str = ""
) -> int:
    from database import CalibrationVersion, Camera, ZoneConfig
    ...
    if frame is not None:
        snapshot_path = os.path.join(
            CALIBRATION_DIR, f"camera_{camera_id}_v{version}.jpg"
        )
    ...
    db.add(
        CalibrationVersion(
            ...
            reference_frame_path=snapshot_path or camera.reference_frame_path,
...
def assess_drift(
    camera, frame: np.ndarray, threshold: float = 18.0
    ...
    displacement = [
        float(np.linalg.norm(np.array(k1[m.queryIdx].pt) - np.array(k2[m.trainIdx].pt)))
        for m in matches
    ]
