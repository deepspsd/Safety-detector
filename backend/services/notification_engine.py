...
class NotificationEngine:
    ...
    def deliver(self, event: Event) -> None:
        ...
        outcomes = []
        try:
            case = db.query(AlertCase).filter(AlertCase.id == payload.get("alert_case_id")).first()
            camera = db.query(Camera).filter(Camera.id == event.camera_id).first() if event.camera_id else None
            for target in self._normalise_targets(targets):
                delivery = NotificationDelivery(
                    alert_case_id=case.id if case else None,
                    channel=target["channel"], target=target.get("target"),
                    status="queued", payload_json=json.dumps(payload, default=str),
                )
                db.add(delivery)
                db.flush()
                self._send(delivery, target, payload, camera)
                outcomes.append({"delivery_id": delivery.id, "channel": target["channel"], "status": delivery.status})
            db.commit()
        except Exception:
            db.rollback()
        finally:
        ...
        for outcome in outcomes:
            emit("NOTIFICATION_DELIVERED" if outcome["status"] == "delivered" else "NOTIFICATION_FAILED",
                 camera_id=event.camera_id, correlation_id=str(payload.get("alert_case_id") or ""),
                 source="notification-engine", payload=outcome)
    ...
    def _normalise_targets(targets: Iterable[Any]):
        for target in targets:
            if isinstance(target, str):
                ...
                yield {"channel": str(target["channel"]).lower(), "target": target.get("target")}

    def _send(self, delivery, target: Dict[str, Any], payload: Dict[str, Any], camera) -> None:
        ...
        if channel in {"dashboard", "push"}:
            ...
            sent = send_telegram_alert(
                ...
                floor=getattr(camera, "floor", None), camera_name=getattr(camera, "name", None),
            )
            delivery.status = "delivered" if sent else "failed"
            delivery.delivered_at = delivery.attempted_at if sent else None
            delivery.error_message = None if sent else "Telegram provider was unavailable or rejected delivery"
        else:
