...
class ContextEngine:
    ...
    def build(self, camera_id: int, tracks: List[TrackObservation], detections: List[Detection], db) -> List[TrackContext]:
        ...
        zones = [] if camera.calibration_status == "required" else (
            db.query(ZoneConfig).filter(ZoneConfig.camera_id == camera_id, ZoneConfig.is_active.is_(True))
            .order_by(ZoneConfig.priority.desc()).all()
        )
        contexts: List[TrackContext] = []
        for track in tracks:
            zone = self._resolve_zone(track.bbox, zones)
            assigned = [item for item in detections if self._belongs(track.bbox, item.bbox)]
            ppe = {
                "present": [x.label for x in assigned if not x.label.startswith("NO-") and x.label in {"Bakery-Head-Cap", "Hardhat", "Mask", "Safety Vest"}],
                "missing": [x.label for x in assigned if x.label.startswith("NO-")],
                "violations": [x.label for x in assigned if x.label in {"Bangles", "Exposed-Item"}],
            }
            context = TrackContext(
                camera_id=camera_id, track_id=track.track_id, occurred_at=track.last_seen_at,
                zone_id=zone.id if zone else None, zone_name=zone.zone_name if zone else None,
                ...
                position=track.bbox, velocity=track.velocity, direction=track.direction,
                movement_state=track.movement_state, objects=[x.to_dict() for x in assigned], ppe=ppe,
            )
    ...
    def _emit_context_events(self, context: TrackContext) -> None:
        ...
        base = dict(camera_id=context.camera_id, track_id=context.track_id, zone_id=context.zone_id,
                    calibration_version=context.calibration_version, payload=payload, source="context-engine")
        emit("PERSON_DETECTED", **base)
        if old_zone != context.zone_id:
            if old_zone is not None:
                emit("ZONE_EXITED", camera_id=context.camera_id, track_id=context.track_id, zone_id=old_zone,
                     calibration_version=context.calibration_version, payload=context.to_dict(), source="context-engine")
        ...
        for label in context.ppe["missing"]:
            emit("PPE_MISSING", confidence=None, **signal_base, payload={**context.to_dict(), "label": label})
        for label in context.ppe["violations"]:
            emit("OBJECT_POLICY_SIGNAL", confidence=None, **signal_base, payload={**context.to_dict(), "label": label})

    @staticmethod
    def _persist_context(context: TrackContext, db) -> None:
        try:
            from database import ContextSnapshot
            db.add(ContextSnapshot(camera_id=context.camera_id, track_id=context.track_id, occurred_at=context.occurred_at,
                                   zone_id=context.zone_id, workflow_stage=context.workflow_stage,
