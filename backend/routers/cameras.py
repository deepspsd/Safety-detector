...
@router.post(
    "/hikvision-quick-add", summary="Add a HikVision camera by IP + credentials"
)
def hikvision_add(
    ...
    cam = CameraModel(**{k: v for k, v in cam_dict.items() if hasattr(CameraModel, k)})
    ...
    try:
        camera_manager.start_camera(
            cam.id, cam.name, cam_dict["rtsp_url"], floor=cam.floor
        )
...
def _camera_to_dict(cam: CameraModel, live: Optional[dict] = None) -> dict:
    ...
    d = {
        ...
        "last_calibrated_at": (
            cam.last_calibrated_at.isoformat() if cam.last_calibrated_at else None
        ),
        "rule_profile_id": cam.rule_profile_id,
        "workflow_profile_id": cam.workflow_profile_id,
        "ai_enabled": cam.ai_enabled,
        "capabilities": {
            "multi_zone": cam.supports_multi_zone,
            "ocr": cam.supports_ocr,
            "pose": cam.supports_pose,
            "tracking": cam.supports_tracking,
            "recording": cam.supports_recording,
            "snapshot": cam.supports_snapshot,
        },
...
def register_onvif_camera(
    ...
    """
    try:
        inspected = inspect_camera(
            payload.onvif_endpoint, payload.username, payload.password
        )
    ...
    cam = CameraModel(
        ...
        zone_type=payload.zone_type,
        status="offline",
        manufacturer=inspected.get("manufacturer"),
    ...
    for stream in inspected["streams"]:
        db.add(
            CameraStreamProfile(
                camera_id=cam.id,
                profile_token=stream.get("profile_token"),
...
def _start_selected_stream(cam: CameraModel, db: Session) -> None:
    try:
        camera_manager.start_camera(
            cam.id, cam.name, _stream_url(cam, db), floor=cam.floor
        )
        cam.status = "online"
        db.commit()
    except CredentialConfigurationError as exc:
...
def camera_health(
    ...
    return {
        "camera_id": camera_id,
        "status": cam.health_status or cam.status,
        **metrics,
    }
...
def select_camera_stream(
    camera_id: int,
    stream_type: str = Query(..., pattern="^(main|sub)$"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    cam = db.query(CameraModel).filter(CameraModel.id == camera_id).first()
    ...
    camera_manager.restart_camera(
        cam.id, cam.name, _stream_url(cam, db, profile.stream_type), floor=cam.floor
    )
    cam.status = "online"
    db.commit()
    return {
...
def update_camera(
    ...
    if url_changed and cam.rtsp_url and cam.status != "offline":
        try:
            camera_manager.restart_camera(
                cam.id, cam.name, cam.rtsp_url, floor=cam.floor
            )
...
def create_or_replace_zone(
    ...
    existing = (
        db.query(ZoneConfig)
        .filter(
            ZoneConfig.camera_id == camera_id, ZoneConfig.zone_name == payload.zone_name
        )
        .first()
    )
    if existing:
        ...
        for field, value in {
            ...
            "rule_profile_id": payload.rule_profile_id,
            "expected_objects_json": (
                _json.dumps(payload.expected_objects)
                if payload.expected_objects is not None
                else None
            ),
            "allowed_objects_json": (
                _json.dumps(payload.allowed_objects)
                if payload.allowed_objects is not None
                else None
            ),
            "forbidden_objects_json": (
                _json.dumps(payload.forbidden_objects)
                if payload.forbidden_objects is not None
                else None
            ),
            "time_constraints_json": (
                _json.dumps(payload.time_constraints)
                if payload.time_constraints is not None
                else None
            ),
            "alert_thresholds_json": (
                _json.dumps(payload.alert_thresholds)
...
def discover_onvif_cameras(
    ...
    devices = discover_onvif(
        payload.timeout_seconds, payload.interface, payload.retries
    )
    fallback: list[dict] = []
    ...
    log.info(
        "ONVIF discovery returned %s validated device(s), %s candidate(s)",
        len(devices),
        len(fallback),
    )
    return {
        "devices": values,
        "onvif_devices": len(devices),
